...
SELECT SlaStatus, BusinessHoursToCarton, HoursToCarton, SlaDueAt, ReceivedAtDC,
       FirstCartonAt, ReceiptAnchorSource, ReceiptSpreadHours, ShipClockSource, SlaBreachFlag,
       AwaitingReceiptDays,
       TraceStatus, PO, Store, DC, DestType, Receiver, ItemId, Color, Size, ASN, IAAllocationNumber,
       AllocatedQty, CartonedQty, ReceivedQty, AllocGapQty, TransitGapQty,
       Cartons, CartonNumbers, QtyInOpenCartons, AgeDays, ReceiveByException,
       AllocatedDate, FirstShippedDate, LastReceivedDate
FROM trace
-- Default to the action list: everything past the SLA, worst first. Comment out to see all rows.
WHERE SlaBreachFlag = 1
ORDER BY BusinessHoursToCarton DESC;
