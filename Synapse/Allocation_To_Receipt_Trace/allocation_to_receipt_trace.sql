...
SELECT DestType, SlaStatus, TraceStatus,
       COUNT_BIG(*)               AS lines_,
       SUM(AllocatedQty)          AS allocated_u,
       SUM(CartonedQty)           AS cartoned_u,
       SUM(ReceivedQty)           AS received_u,
       SUM(AllocGapQty)           AS alloc_gap_u,
       SUM(TransitGapQty)         AS transit_gap_u,
       AVG(BusinessHoursToCarton) AS avg_biz_hours
FROM trace
GROUP BY DestType, SlaStatus, TraceStatus
ORDER BY DestType, SlaStatus, TraceStatus;
