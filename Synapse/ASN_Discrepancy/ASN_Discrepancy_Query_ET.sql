...
-- Timezone: Eastern Time (handles EDT/EST automatically)
-- Change @ReportDate to the date you want to investigate (ET)
-- ============================================================
...
WITH pix AS (
    ...
    WHERE CAST(createddatetime AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS DATE) = @ReportDate
      AND pxtxtp = '606'
      AND pxtxcd = '03'
...
d365 AS (
    ...
    WHERE T.dataareaid          = '1001'
      ...
      AND CAST(T.posteddatetime AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time' AS DATE) = @ReportDate
